...
class DetectFake:
    ...
    def initialization_vector(self) -> Union[TfidfVectorizer, Tokenizer]:
        ...
        logging.info("Vector for passive-aggressive model")
    ...
    def prepare_data(self):
        ...
        logging.info("Preparing data for passive-aggressive modelChoose the model")
    ...
    def vectorization_of_text(self):
        ...
        logging.info("Vectorizating of text for passive-aggressive")

        self.vectorization_data = {
            "vec_train": self.init_vectorizer.fit_transform(
                self.data_collection["x_train"].values.astype("U")
            ),
            "vec_test": self.init_vectorizer.transform(
                self.data_collection["x_test"].values.astype("U")
            ),
        }
    ...
    def passive_aggressive_classifier(self):
        ...
        logging.info("Fitting and save passive aggressive model")
    ...
    def accuracy(self):
        ...
        print(
            confusion_matrix(
                self.data_collection["y_test"], y_pred, labels=["Real", "Fake"]
            )
        )
    ...
    def cleaning_news_for_neural_net(self):
        ...
        logging.info("Dropping and uniting text for neural network")

        self.data = self.data.drop(columns=["id", "title", "author"], axis=1)
        self.data = self.data.dropna(axis=0)
        if not self.model_path:
            self.data["clean_news"] = self.data["text"].str.lower()
            self.data["clean_news"] = self.data["clean_news"].str.replace(
                r"[^A-Za-z0-9\s]", ""
            )
            ...
            self.data["clean_news"] = self.data["clean_news"].replace(
                r"[^A-Za-z0-9\s]", ""
            )
    ...
    def vector_text_for_neural_network(self):
        ...
        logging.info("Fitting vectorization and creating padded sequence")
        ...
        self.padded_seq = pad_sequences(
            sequences, maxlen=600, padding="post", truncating="post"
        )
    ...
    def create_matrix(self):
        ...
        logging.info("Creating embedded matrix based on glove 6B")
    ...
    def init_test_train_split(self):
        ...
        logging.info("initializating of test and train sets")
    ...
    def neural_network(self):
        ...
        logging.info("Creating neural network and fitting data")
        ...
        print(model.summary())
        model.fit(
            self.data_collection["x_train"],
            self.data_collection["y_train"],
            epochs=10,
            batch_size=256,
            validation_data=(
                self.data_collection["x_test"],
                self.data_collection["y_test"],
            ),
        )
        self.model = model
    ...
    def predict(self, newtext: Series) -> str:
        ...
        logging.info("Predicting by passive-aggressive model")
    ...
    def predict_by_neural_network(self) -> float:
        ...
        logging.info("Predicting by neural network")
